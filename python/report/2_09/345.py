...
outfile = open("indata-1.txt", "w")

def main() :
    scores = readFloats()
    removeMinimum(scores, 2)        # scores에서 최솟값을 두 번 삭제하는 함수입니다.
    total = sum(scores)
    print("결과 : %.2f " %total)      # 결과값을 소수점 두번째 자리까지 출력합니다.
...
def readFloats() :
    ...
    quizValues = input("퀴즈 점수의 개수를 입력해주세요 3 ~ 12 :")
    ...
    while trueFalse == False :
        if int(quizValues) > 2 and int(quizValues) < 13 :       # 퀴즈 점수의 개수를 3부터 12까지의 정수로 입력받습니다. 
            trueFalse = True
            while count < (int(quizValues)) :                   # 퀴즈 점수의 개수를 입력받았으면, 퀴즈 점수를 개수만큼 입력받습니다.
                try :
